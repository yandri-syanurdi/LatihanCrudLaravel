<?php include 'config.php';
	...
	if($_GET['id']!="") {
		...
		if (mysql_num_rows($result) > 0) {
			$messages_result = $link->mysql_query("SELECT SQL_NO_CACHE * FROM messenger_messages WHERE (from_id='$id' AND to_id='$contact_id') OR (from_id='$contact_id' AND to_id='$id') ORDER BY message_id");
			$messages = array();
